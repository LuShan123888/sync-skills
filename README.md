...
2. **link** -- 将 Agent 目录中已有的野生 Skill 纳入管理（复制到 git 仓库 + 创建 symlink）
...
| `sync-skills link [name]` | 纳入野生 Skill（省略 name 则列出可收养的 Skill，-y 跳过确认） |
...
2. **link** -- Link an existing wild skill from agent directories into management (copy to git repo + create symlinks)
...
| `sync-skills link [name]` | Link a wild skill into management (omit name to list, -y to skip confirmation) |
