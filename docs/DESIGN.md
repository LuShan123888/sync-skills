...
**为什么不直接从目标 A 分发到目标 B，而是先收集到源？**
1. 单一真实来源：源目录是所有 skill 的规范位置，收集后后续同步只需走统一的分发路径，避免每次重复发现"新 skill"
2. 分类管理：收集到 `~/Skills/Other/` 后，用户可将 skill 移到合适分类（如 `Code/`、`Lark/`）
3. 代码复用：分发阶段（源→目标）是统一代码路径，无需额外维护 target→target 的分发逻辑
...
| 8 | 双向模式不自动推送源修改 | ✅ 已解决：源修改时自动识别为 singleton（唯一不同的版本），自动分发 |
...
- `tests/test_sync_skills.py`：新增 `TestDryRun` 测试类（5 个测试）
