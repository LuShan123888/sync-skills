...
- `doctor --dry-run` 还不是真正纯预览
- `status` 还不会显式报告真实目录冲突
- v1 `list` 与 legacy `list` 还存在入口竞争
...
### P0-1 `doctor --dry-run` 真正只读

将 `doctor --dry-run` 定义为纯预演模式。执行时只输出拟修复动作、跳过原因和无操作结果，不允许改写 state，不允许创建或删除 symlink，也不允许修改目录结构。没有待修复项时应直接成功退出，不再要求额外确认。

### P0-2 `status` 生命周期状态补全

`status` 需要显式报告 `managed`、`unknown`、`broken link`、`real directory conflict`、`orphaned`、`managed but not exposed` 等状态，让它成为生命周期健康视图，而不是简单列表输出。

### P0-3 state 残留与 orphaned 收口

为 orphaned 和残留 state 建立明确修复路径。`status` 负责可见性，`doctor` 负责收口与修复，状态转移规则需要在实现、测试和文档中保持一致。

### P1-1 Git 异常恢复路径

当前已经具备 `commit`、`push`、`pull` 主流程，但还需要补强失败场景处理，包括远程未配置、首次 upstream 建立、本地领先或落后、分叉、冲突、无改动提交和 Git 不可用等情况。目标不是自动处理所有问题，而是准确识别、明确提示并安全退出。
...
以下能力不属于当前主线，因此不纳入本阶段 roadmap：

- 单个 Skill 级别的 `publish`
- 单个 Skill 级别的 `import`
- 单个 Skill 级别的 `install-from-git`
- 搜索别人发布的 Skill
- 中心化 Skill registry 或 marketplace
